...
namespace Capa_Datos
{
    ...
    public class ReporteStockDatos { public int IdProducto { get; set; } public int IdSucursal { get; set; } public string Codigo { get; set; } = ""; public string Producto { get; set; } = ""; public string Categoria { get; set; } = ""; public string Sucursal { get; set; } = ""; public int Stock { get; set; } public int StockMinimo { get; set; } }
    public class ReporteVentaDatos { public int IdVenta { get; set; } public DateTime Fecha { get; set; } public string Cliente { get; set; } = ""; public string Vendedor { get; set; } = ""; public string Sucursal { get; set; } = ""; public decimal Subtotal { get; set; } public decimal Descuento { get; set; } public decimal Total { get; set; } public string MetodosPago { get; set; } = ""; }
    ...
    public class ReporteDatos
    {
        ...
        public List<ReporteStockDatos> ObtenerStock(int? sucursal) => Lista(ComandoStock(sucursal), r => new ReporteStockDatos { IdProducto=r.GetInt32(0), IdSucursal=r.GetInt32(1), Codigo=r.IsDBNull(2)?"":r.GetString(2), Producto=r.GetString(3), Categoria=r.GetString(4), Sucursal=r.GetString(5), Stock=r.GetInt32(6), StockMinimo=r.GetInt32(7) });
        // Obtiene el detalle de ventas sin cargar información de pagos o ítems innecesaria.
        public List<ReporteVentaDatos> ObtenerDetalle(DateTime desde, DateTime hasta, int? sucursal, int? usuario) => Lista(Comando("dbo.sp_Reporte_DetalleVentas", desde, hasta, sucursal, usuario), r => new ReporteVentaDatos { IdVenta=r.GetInt32(0), Fecha=r.GetDateTime(1), Cliente=r.GetString(2), Vendedor=r.GetString(3), Sucursal=r.GetString(4), Subtotal=r.GetDecimal(5), Descuento=r.GetDecimal(6), Total=r.GetDecimal(7), MetodosPago=r.IsDBNull(8)?"":r.GetString(8) });
